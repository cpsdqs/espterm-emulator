...
impl CursorState {
  fn new() -> CursorState {
    CursorState { x: 0, y: 0, style: 1, visible: true }
  }
}
...
impl Terminal {
  ...
  pub fn clear_screen(&mut self) {
    self.state.buffer.clear(self.width as usize, self.height as usize, self.state.style.to_bytes());
  }
  ...
  pub fn scroll (&mut self, amount: i32, with_cursor: bool) {
    for y in 0..self.height {
      if (y as i32) + amount < 0 || ((y as i32) + amount) as u32 >= self.height {
        ...
        self.state.buffer.lines[y as usize] = self.state.buffer.clone_line(((y as i32) + amount) as usize);
      }
    }
    if with_cursor {
  ...
  pub fn delete_lines (&mut self, count: u32) {
    for y in (self.state.cursor.y as u32)..self.height {
      if y + count >= self.height {
        self.state.buffer.lines[y as usize] = ScreenBuffer::make_line(self.width as usize, 0);
      } else {
        self.state.buffer.lines[y as usize] = self.state.buffer.lines[(y + count) as usize].clone();
      }
    }
